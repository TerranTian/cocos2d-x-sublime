...
def parseLua(file):
    ...
    while True:
        ...
        if line:
            ...
            if m:
                saveFunction(saveDir,"",m.group(1),m.group(2),filename)
            ...
            if m:
                ...
                if method=="ctor":
                    continue
                saveFunction(saveDir,m.group(1),method,m.group(3),filename)
            ...
            break
    f.close()
    saveCompletions(completionsList,saveDir,filename)

def fixMname(name,filename):
    return re.sub(r"^M\b([\.:]?)",lambda n : filename+n.group(1), name)
 
def handleDefinition(function,param,file,lineNum,showFunc=None):
    filename = os.path.basename(file).split(".")[0]
    function = fixMname(function,filename);
...
def saveFunction(saveDir,classname,function,param,filename): 
    arr=handleParam(param)
    
    function = fixMname(function,filename);
    classname = fixMname(classname,filename);
